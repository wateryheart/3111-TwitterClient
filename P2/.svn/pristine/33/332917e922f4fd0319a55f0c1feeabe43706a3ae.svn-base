...
public class TweetView extends RowComposite {
  ...
  private void initialize(int width, boolean bigIcon, Color origColor, Color hoverColor, Color clickedColor) {
    ...
    // set text
    m_link = new Link(m_rightFrame,SWT.WRAP | SWT.LEFT);
    m_link.setFont(Resources.FONT11);
    m_link.setForeground(Resources.TEXT_COLOR);

    //set text with link
    m_link.setText(getDisplayText(m_tweet));
    m_link.setLayoutData(new RowData(getBounds().width - 87, -1));
    m_link.addListener(SWT.Selection, new Listener(){

      @Override
      public void handleEvent(Event event) {
          //System.out.println(event.text);

          //open link in a external web browser
...
//    Utils.cutRoundCorner(m_iconImage, true, true, true, true);
  ...
  private String getDisplayText(Status tweet) {
    ...
    for (URLEntity urlEntity : urlEntities) {
        ...
        // use html style to represent the link.
        text = text.replace(urlEntity.getURL(), "<a href=\"" + urlEntity.getExpandedURL() + "\">" + urlEntity.getDisplayURL() +"</a>" );
    }
    return text;
  }
