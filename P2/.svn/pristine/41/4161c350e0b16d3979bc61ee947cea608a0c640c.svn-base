...
import hk.ust.cse.TwitterClient.reply.reply;
...
//    Utils.cutRoundCorner(m_iconImage, true, true, true, true);
    ...
    private void setLowerFrame() {
        ...
        if(m_tweet.getUser().getId() == TwitterControl.getAccountUser().getId()) {
        	...
        	m_statusDeleteIcon = Utils.loadImageFromLocal(Resources.class, "./status_cell_delete_hover_icon.png");
  ...
  public void showReplies(List<Status> replies) {
    ...
    m_reply = new reply(m_extendFrame, m_user,m_tweet,430,150);
