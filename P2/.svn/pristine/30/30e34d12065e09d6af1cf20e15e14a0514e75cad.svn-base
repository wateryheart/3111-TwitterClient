...
public class ControlBar extends RowComposite {
  ...
  private void initialize(int width, int height, int initLeftMargin) {
    ...
    // set DM frame
    RowComposite DMFrame = new RowComposite(this, SWT.RIGHT, SWT.HORIZONTAL, false, 10, 0, 450, 0, 40);
    DMFrame.setBackground(Resources.CONTROL_BAR_COLOR);
    DMFrame.setLayoutData(new RowData(-1, height));
    
    // set DM button
    m_dm = new ControlBarItem(DMFrame, "Direct Message", Resources.DM_IMG, Resources.DM2_IMG);
    m_dm.setLayoutData(new RowData(Resources.DM_IMG.getBounds().width, 
                                   Resources.DM_IMG.getBounds().height));
    Utils.addClickListener(m_dm, m_btnClkHandler, m_handlerCallee);
