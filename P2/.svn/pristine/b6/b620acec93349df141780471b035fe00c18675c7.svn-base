...
public class HomePage extends RowComposite {
  ...
  private void initialize(int width, int miniProfileWidth, int miniProfileHeight, 
    ...
    postBoxHeight = 150;
    m_postBox = new PostBox(leftFrame, m_user, miniProfileWidth,postBoxHeight);
  ...
  public void showTweetsList(List<Status> tweets) {
    System.out.println("HomePage showTweetsList");
    try{

        if(isDisposed()){
            Mutex.releaseHomePageMutex();
            System.out.println("releaseHomePageMutex because HomePage is disposed.");
    ...
    if (getBounds().height < m_minPageHeight) {
      setSize(m_width, m_minPageHeight);
        ...
        Mutex.releaseHomePageMutex();
        System.out.println("releaseHomePageMutex");
