...
public class PostBox extends RowComposite {	
	...
	private class MListener implements MouseListener{
		public void mouseDown(MouseEvent me) {
			...
			try {
				...
				status = twitter.updateStatus(constructStatus());
	...
	*/
    ...
    private StatusUpdate constructStatusWithLocation(){
        StatusUpdate status = this.constructStatus();
        status.location(this.getCurrentUserLocation());
        return status;
    }

    private StatusUpdate constructStatus(){
        StatusUpdate status = new StatusUpdate(input.getText());
        return status;
    }

    private GeoLocation getCurrentUserLocation(){
        CityLookUp c = new CityLookUp();
        return new GeoLocation(c.getLatitude(), c.getLongitude());
    }
