...
public class HashTagTweetFeature extends  BaseTweetFeature{
    ...
    public void BuildData(String text) {
        ...
        if(_hashTagEntities != null){
            if(_hashTagEntities.length > 0){
                ...
                while (matcher.find()){
                    logger.debug( String.format("TagName: %s, regionStart: %d, regionEnd: %d, start: %d, end: %d", matcher.group(), matcher.regionStart(), matcher.regionEnd(), matcher.start(), matcher.end()));
    ...
    private List<String> getDistinctHashTags() {
        ...
        logger.debug( Arrays.toString(_hashTagEntities));
        return distinctList;
    }

    @Override
    public boolean eventHandle(Event event) {
        logger.debug("=> HashTagTweetFeature.eventHandle");
