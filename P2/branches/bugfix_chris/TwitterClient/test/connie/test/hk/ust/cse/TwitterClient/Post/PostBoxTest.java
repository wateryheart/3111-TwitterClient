...
public class PostBoxTest {
	...
	public void testConstructor() throws Throwable, TwitterException {
	    ...
	    PostBox post = new PostBox(shell, user, 300,150,null);
	    assertNotNull(post);
	    ...
	    input.insert("input something, such as english, abcd efghij, BN, Fm,,/?12345,");
	    input.insert("%^$#@*&,><=-):][");
	    ...
	    /*
	    Field fStatus = cls.getDeclaredField("status");
	    fStatus.setAccessible(true);
	    Object objStatus = fStatus.get(post);
	    assertNotNull(objStatus);
	    ...
	    */
		...
		TwitterControl.setupTwitter(twitter, AsyncTwitter);
	    post = new PostBox(shell, user, 300,150,null);
	    post.dispose();
	    assertNotNull(post);
	    shell.dispose();
		display.dispose();
