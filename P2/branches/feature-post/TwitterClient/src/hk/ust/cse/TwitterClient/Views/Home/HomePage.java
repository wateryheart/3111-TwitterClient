...
public class HomePage extends RowComposite {
  ...
  private void initialize(int width, int miniProfileWidth, int miniProfileHeight, 
    ...
    final RowComposite leftFrame = new RowComposite(this, 0, SWT.VERTICAL, true, 0, 0, 0, 0, 0);
    ...
    //switch size
    leftFrame.addListener(SWT.MouseDown,new Listener()
    {
    	public void handleEvent(Event e) {
    		if (e.keyCode == 32 && !leftFrame.isDisposed()) {
    			leftFrame.setLayoutData(new RowData(300,300));    			
    			layout();
    			pack();    			
  ...
  public void refreshHome(Status status){
	  m_tweetPage = 1;
	  TwitterControl.getUserTimeline(m_user.getScreenName(), m_tweetPage, "showTweetsListCallback", m_control);
  }
