...
public class PostBox extends RowComposite {	
	...
	private class TestListener implements ModifyListener{
		public void modifyText(ModifyEvent me)  {
			//TODO: Do not count space at end
	...
	private class FListener implements FocusListener{
		...
		public void focusGained(FocusEvent fe) {
			PostBox.this.setLayoutData(new RowData(m_width, 150));
			Event e = new Event();
			e.keyCode = 32;
			top.setLayoutData(new RowData(m_width-20, 100));
			input.setLayoutData(new RowData(m_width - 50, 90));
			input.setForeground(Resources.TEXT_COLOR);
			input.setFont(Resources.FONT11);
			...
			//TODO: Show words and update after focus
			layout();
			pack();
			m_parentView.notifyListeners(SWT.MouseDown,e);
		}
	
		public void focusLost(FocusEvent fe) {
			//TODO: Hide it as original if post box has no entry/only blank/only original mention
			if (words.getText().equals(String.valueOf(ori)))
			{
			...
			if (count == ori)
			{
				...
				top.setLayoutData(new RowData(m_width-20, 50));
				//set input message
			    box = new Text(top, SWT.BORDER |SWT.FILL);
			    box.setSize(m_width,m_height);
	...
	*/
	private void initialize(int width,int height) 
	 {
	    ...
	    this.setLayoutData(new RowData(width, 50));
	    top = new RowComposite(this, 0, SWT.HORIZONTAL, false, 10, 10, 5, 5, 10);
	    top.setLayoutData(new RowData(width-20, 40));
	    ...
	    */
	    ...
	    box.setEnabled(true);
	    box.addFocusListener(new FListener());
	    box.setLayoutData(new RowData(m_width - 50, m_height));

	    layout(); // trigger re-layout
	    pack(); // force re-size
	    */
	    
	    input = new Text(top,560);
		//input = new Text(top,  SWT.BORDER|SWT.WRAP |SWT.MULTI|SWT.V_SCROLL);
		input.setEditable(true);
		input.setText("Compose new Tweet...");
		input.setFont(Resources.FONT14);
		input.setForeground(Resources.GRAY_COLOR);
		input.addModifyListener(new TestListener());
		input.setLayoutData(new RowData(width - 50, 30));
		input.addFocusListener(new FListener());
		
		down = new RowComposite(this, 0, SWT.HORIZONTAL, false, 5, 10, 5, 5, 10);
	    down.setLayoutData(new RowData(m_width-20, 50));
	    down.setVisible(false);
		
	    warning = new Label(down, SWT.LEFT|SWT.WRAP);
	    warning.setForeground(Resources.RED_COLOR);
	    warning.setVisible(false);
	    warning.setLayoutData(new RowData(m_width-130,30));
	...
	RowComposite top;
	RowComposite down;
	private Composite m_parentView;
	private int m_width;
