...
import hk.ust.cse.TwitterClient.Views.*;
...
public class UserPage extends RowComposite {
  ...
  private void initialize(int width, 
	      ...
	      try{
              addPaintListener(new PaintListener() {
                  public void paintControl(PaintEvent e) {
                      if (!UserPage.this.m_user.isProfileBackgroundTiled()) {
                          e.gc.drawImage(UserPage.this.m_bgImage, 0, 0);
                      } else {
                          UserPage.this.setBackgroundImage(UserPage.this.m_bgImage);
                      }
                  }
              });
	      }
	    ...
	    final RowComposite postFrame = new RowComposite(m_leftFrame, 0, SWT.VERTICAL, false, 5, 5, 5, 5, 15);
	    ...
	    //postBoxHeight = 150;
	    m_postBox = new PostBox(postFrame, m_user, menuWidth-20,postBoxHeight,m_handlerCallee);
	    //m_postBox.setLayoutData(new RowData(menuWidth-20, postBoxHeight));
	    
		  //switch size
	    postFrame.addListener(SWT.MouseDown,new Listener()
		    {
		    	public void handleEvent(Event e) {
		    		if (e.keyCode == 32 && !postFrame.isDisposed()) {
		    			postFrame.setLayoutData(new RowData(300,300));    			
		    			layout();
		    			pack();    			
		    			//TODO: extend size
		    			Utils.cutRoundCorner(m_leftFrame, true, true, true, true); 
		    			Utils.cutRoundCorner(postFrame, true, true, true, true);    		    
		    		    Utils.cutRoundCorner(m_postBox, true, true, true, true);//for postBox
		    		}
		    	}
		    });
  ...
  public void showTweetsList(List<Status> tweets) {
      logger.log(Level.INFO, "UserPage showTweetsList");
      ...
      if(isDisposed()){
          ...
          try {
              ((WholePage)m_handlerCallee).gotoUserPage(m_user);
          } catch (TwitterException e) {
    ...
    public void showFollowingList(PagableResponseList<User> following) {
        try {
            ...
            if (isDisposed()) {
                ...
                ((WholePage) m_handlerCallee).gotoUserPage(m_user, "following");
                return;
            }
            // remove the old list view
            Utils.dispose(m_itemList);
  ...
  public void showFollowersList(PagableResponseList<User> followers) {
      ...
      logger.log(Level.INFO, "UserPage showFollowersList");
      try {
          ...
          if (isDisposed()) {
              ...
              ((WholePage) m_handlerCallee).gotoUserPage(m_user, "followers");
              return;
          }
          // remove the old list view
          Utils.dispose(m_itemList);
