package connie.test.hk.ust.cse.TwitterClient.Views;

import static org.junit.Assert.*;


import java.lang.reflect.Method;
import java.text.SimpleDateFormat;
import java.util.ArrayList;
import java.util.Calendar;
import java.util.Date;
import java.util.List;
import java.util.Locale;

import connie.test.hk.ust.cse.TwitterClient.Mocks.MockAsyncTwitter;
import connie.test.hk.ust.cse.TwitterClient.Mocks.MockTwitter;
import hk.ust.cse.TwitterClient.Controls.TwitterControl;
import hk.ust.cse.TwitterClient.Resources.Resources;
import hk.ust.cse.TwitterClient.Views.NumberBar;
import hk.ust.cse.TwitterClient.Views.TweetView;

import org.eclipse.swt.events.MouseEvent;
import org.eclipse.swt.widgets.Display;
import org.eclipse.swt.widgets.Event;
import org.eclipse.swt.widgets.Shell;
import org.junit.Test;

import twitter4j.AsyncTwitter;
import twitter4j.Status;
import twitter4j.Twitter;
import twitter4j.json.DataObjectFactory;

public class TweetViewTest {

	@Test(timeout=10000)
	public void testTweetView()throws Throwable {
		Display display = new Display();
	    Shell shell = new Shell(display);

        Twitter twitter = new MockTwitter( );
        AsyncTwitter asyncTwitter = new MockAsyncTwitter();
        TwitterControl.setupTwitter(twitter, asyncTwitter);

	    Status status;
	    status = DataObjectFactory.createStatus("{\"text\":\"\\\\u5e30%u5e30 &lt;%}& foobar &lt;&Cynthia&gt;\",\"contributors\":null,\"geo\":null,\"retweeted\":false,\"in_reply_to_screen_name\":null,\"truncated\":false,\"entities\":{\"urls\":[],\"hashtags\":[],\"user_mentions\":[]},\"in_reply_to_status_id_str\":null,\"id\":12029015787307008,\"in_reply_to_user_id_str\":null,\"source\":\"web\",\"favorited\":false,\"in_reply_to_status_id\":null,\"in_reply_to_user_id\":null,\"created_at\":\"Tue Dec 07 06:21:55 +0000 2010\",\"retweet_count\":0,\"id_str\":\"12029015787307008\",\"place\":null,\"user\":{\"location\":\"location:\",\"statuses_count\":13405,\"profile_background_tile\":false,\"lang\":\"en\",\"profile_link_color\":\"0000ff\",\"id\":6358482,\"following\":true,\"favourites_count\":2,\"protected\":false,\"profile_text_color\":\"000000\",\"contributors_enabled\":false,\"description\":\"Hi there, I do test a lot!new\",\"verified\":false,\"profile_sidebar_border_color\":\"87bc44\",\"name\":\"twit4j\",\"profile_background_color\":\"9ae4e8\",\"created_at\":\"Sun May 27 09:52:09 +0000 2007\",\"followers_count\":24,\"geo_enabled\":true,\"profile_background_image_url\":\"http://a3.twimg.com/profile_background_images/179009017/t4j-reverse.gif\",\"follow_request_sent\":false,\"url\":\"http://yusuke.homeip.net/twitter4j/\",\"utc_offset\":-32400,\"time_zone\":\"Alaska\",\"notifications\":false,\"friends_count\":4,\"profile_use_background_image\":true,\"profile_sidebar_fill_color\":\"e0ff92\",\"screen_name\":\"twit4j\",\"id_str\":\"6358482\",\"profile_image_url\":\"http://a3.twimg.com/profile_images/1184543043/t4j-reverse_normal.jpeg\",\"show_all_inline_media\":false,\"listed_count\":3},\"coordinates\":null}");
	 	    
	    TweetView tweetView = new TweetView(shell, status, 100, true, Resources.WHITE_COLOR, Resources.HOVER_COLOR, Resources.GRAY_COLOR, "name", this);
	    assertNotNull(tweetView);
	    assertEquals(status, tweetView.getTweet());
	    assertEquals(100, tweetView.getWidth());
	    Event event = new Event();
	    event.widget = tweetView;
	    MouseEvent mEvent = new MouseEvent(event);
	    try{tweetView.toggleExpand(mEvent);}catch(Exception e){};
	    tweetView.dispose();
	   
	    status = DataObjectFactory.createStatus("{\"text\":\"\\\\u5e30%u5e30 &lt;%}& foobar &lt;&Cynthia&gt;\",\"contributors\":null,\"geo\":null,\"retweeted\":false,\"in_reply_to_screen_name\":null,\"truncated\":false,\"entities\":{\"urls\":[],\"hashtags\":[],\"user_mentions\":[]},\"in_reply_to_status_id_str\":null,\"id\":12029015787307008,\"in_reply_to_user_id_str\":null,\"source\":\"web\",\"favorited\":false,\"in_reply_to_status_id\":null,\"in_reply_to_user_id\":null,\"created_at\":\"Tue Dec 07 06:21:55 +0000 2010\",\"retweet_count\":0,\"id_str\":\"12029015787307008\",\"place\":null,\"user\":{\"location\":\"location:\",\"statuses_count\":13405,\"profile_background_tile\":false,\"lang\":\"en\",\"profile_link_color\":\"0000ff\",\"id\":6358482,\"following\":true,\"favourites_count\":2,\"protected\":false,\"profile_text_color\":\"000000\",\"contributors_enabled\":false,\"description\":\"Hi there, I do test a lot!new\",\"verified\":false,\"profile_sidebar_border_color\":\"87bc44\",\"name\":\"twit4j\",\"profile_background_color\":\"9ae4e8\",\"created_at\":\"Sun May 27 09:52:09 +0000 2007\",\"followers_count\":24,\"geo_enabled\":true,\"profile_background_image_url\":\"http://a3.twimg.com/profile_background_images/179009017/t4j-reverse.gif\",\"follow_request_sent\":false,\"url\":\"http://yusuke.homeip.net/twitter4j/\",\"utc_offset\":-32400,\"time_zone\":\"Alaska\",\"notifications\":false,\"friends_count\":4,\"profile_use_background_image\":true,\"profile_sidebar_fill_color\":\"e0ff92\",\"screen_name\":\"twit4j\",\"id_str\":\"6358482\",\"profile_image_url\":null,\"show_all_inline_media\":false,\"listed_count\":3},\"coordinates\":null}");
	    tweetView = new TweetView(shell, status, 100, false, Resources.WHITE_COLOR, Resources.HOVER_COLOR, Resources.GRAY_COLOR, null, null);
	    assertTrue(tweetView.isEnabled());
	    long[] num = {123456789,987654321};
		String[] titles = {"a", "b", "c"};
		NumberBar numberBar = new NumberBar(shell, num , titles, 100, 50, 200, Resources.GRAY_COLOR, Resources.HOVER_COLOR, Resources.LINK_COLOR, Resources.FONT10, Resources.FONT11B, true, "numhandler" , null);
		List<Status> list = new ArrayList <Status>();
	    list.add(status); 
	    
	    tweetView.getTweet();
	    tweetView.showReplies(list);
	    tweetView.toggleExpand(null);
	    tweetView.toggleExpand(mEvent);
	    
	    Method m = TweetView.class.getDeclaredMethod("createTimeString", Date.class);
	    m.setAccessible(true);
	    m.invoke(tweetView, new Date(System.currentTimeMillis()));
	    m.invoke(tweetView, new Date(70*1000));
	    m.invoke(tweetView, new Date(System.currentTimeMillis() - 70*1000));
	    m.invoke(tweetView, new Date(System.currentTimeMillis() - 600*1000));
	    m.invoke(tweetView, new Date(System.currentTimeMillis() - 3601*1000));
	    m.invoke(tweetView, new Date(System.currentTimeMillis() - 86401 *1000));	    
		
	    /**
		 * Test for new time stamps.
		 * 1. Before today
		 * 2. Today
		 */
		assertEquals("Dec 07",tweetView.createTimeString(status.getCreatedAt()));
		Date t = Calendar.getInstance().getTime();
		String time = new SimpleDateFormat("HH:mm",Locale.ENGLISH).format(t);
		String createTime = tweetView.createTimeString(t);
		time.equals(createTime);
		tweetView.dispose();

		
	    shell.dispose();
	    display.dispose();
	}


	@Test(timeout=10000)
	public void testToggleExpand()throws Throwable {
		
	}

	@Test(timeout=10000)
	public void testShowReplies()throws Throwable {
		//fail("Not yet implemented");
	}

	@Test(timeout=10000)
	public void testGetTweet()throws Throwable {
		//fail("Not yet implemented");
	}

}
