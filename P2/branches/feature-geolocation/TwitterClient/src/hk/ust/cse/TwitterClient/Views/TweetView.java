...
import hk.ust.cse.TwitterClient.AddressComponents;
...
public class TweetView extends RowComposite {
  ...
  public TweetView(Composite parentView, Status tweet, int width, boolean bigIcon,
    ...
    m_handlerCallee  = handlerCallee;
     logger.log(Level.INFO, m_linkedStatus.toString());
  ...
  private void initialize(int width, boolean bigIcon, Color origColor, Color hoverColor, Color clickedColor) {
    ...
    try{
      ...
      if(m_tweet.getGeoLocation() != null)createLocationLabel();
...
//    Utils.cutRoundCorner(m_iconImage, true, true, true, true);
    ...
    private void setLowerFrame() {
        ...
        m_statusLowerFrame.setLayoutData(new RowData(555, 21));
        ...
        if(!m_tweet.isRetweet() && m_tweet.getUser().equals(TwitterControl.getAccountUser())) {
        	m_statusDeleteLabel= new Label(m_statusLowerFrame, SWT.DOWN);
        ...
        //Add Location Icon
        if(m_tweet.getGeoLocation() != null) {
            Label m_statusLocLabel = new Label(m_statusLowerFrame, SWT.LEFT);
            Image m_statusLocIcon = Resources.POST_GEO_HIGHLIGHTED_ICON;
            m_statusLocLabel.setImage(m_statusLocIcon);
            m_statusLocLabel.setLayoutData(new RowData(21, 15));
        }
    }
  ...
  public void showReplies(List<Status> replies) {
    ...
    private void createLocationLabel() {
        Label m_locDetail = new Label(m_rightUpFrame, SWT.LEFT);
        m_locDetail.setFont(Resources.FONT9);
        GeoCoder gc = new GeoCoder(m_tweet.getGeoLocation().getLatitude(), m_tweet.getGeoLocation().getLongitude());
        String location = "";
        if(gc.getLongName(GeoCoder.Type.ADMINISTRATIVE_AREA_LEVEL_1.value()) == null)
            location = gc.getLongName(GeoCoder.Type.LOCALITY.value());
        else {
            location = gc.getLongName(GeoCoder.Type.ADMINISTRATIVE_AREA_LEVEL_1.value());
        }
        m_locDetail.setText("    From " + location + ", " + gc.getShortName(GeoCoder.Type.COUNTRY.value()));
    }
